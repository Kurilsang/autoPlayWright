...
Agent 产品有 Web 端和 Electron 客户端，页面多、业务链路多。核心流程链路（会话创建、对话、工具调用结果展示、文档处理等）目前完全没有自动化覆盖，全靠手工回归，成本高且不可持续。逐条手写 UI 自动化用例不现实：页面数量大、链路数量多，且 Agent 回复由 LLM 生成、内容非确定，传统"断言固定文本"的用例写法会大量假失败。
...
24. 作为框架维护者，我想让 flow schema 中预留 `judge` 字段且 v1 执行时标记 planned，这样后续接入 LLM-as-Judge 不需要改 DSL 结构和存量用例。
...
steps:
  ...
  - judge:  { ... }                  # v1 预留：校验通过但执行时标记 planned
```
...
- **风险与前置开放项**（进入 M0 前需确认）：
  - ~~登录方式~~ 已解决（2026-09-22）：被测产品为账号密码表单登录，凭据走 `configs/secrets.local.yaml`（gitignored）/环境变量，storage_state 回写复用，见票 01。
...
| D9 | 测试资产分层 | 核心链路进框架资产（DSL/动作原语/定位器仓库）；探索区用 AI 工作区脚本，属临时资产、不入用例库（2026-09-23 grilling） |
| D10 | AI 对测试资产的权限 | AI 只修资产、不改现场：失败后产修复 diff 人工审合入；禁止运行时自适应/自愈（与「失败即取证禁止自愈」同源）（2026-09-23） |
| D11 | 高频变更应对 | 定稿（2026-09-23 Q14）：定位器多候选回退 + 断言打稳定结构标记 + 用例总量控制（v1 20~50 条）**立即执行**；AI 修复 diff 流（=D10 机制）近期工程化；快照 diff 影响清单维持票 08 主交付 |
| D12 | 工作区脚本生命周期 | 用完默认归档为生成器素材/快照基线（`.scratch/`）；升格 `scripts/` 长期资产需人工判定（2026-09-23） |
| D13 | UI 判定主体 | CI 门禁只认确定性结构标记；AI 判定走 `judge` 步出参考意见，不挡门禁（2026-09-23） |
| D14 | 生成器接入 | 先人在环后无人值守：生成侧以 opencode skill/命令为载体（prompt 资产沉淀为 skill），稳定后升级 CLI/SDK 流水线；**pi agent 不引入**（避免双生成方 prompt 漂移），未来确有需要再评估（2026-09-23） |
| D15 | 草稿入库验收门 | 分期：v1 = schema 校验 + ruff + 真实环境绿跑 ≥3 次（防偶发卡死污染结论）；生成器稳定后加固定快照回放测试（票 07 验收项）（2026-09-23） |
| D16 | 动作原语集 | 固定枚举起步：语义原语（含链路级完成判定/取证）+ 基础原语（元素级，动作成功=locator 状态）两档，共 8~12 个；裸 click/fill 不构成链路完成证据，仅用基础原语的 flow 必须以 assert 收口（schema 静态检查）；TODO 演进为开放注册制 + 评审（2026-09-23） |
| D17 | Page 注册机制 | 保持静态注册（`default_registry()` 单点）；生成新页面类的 diff 连带注册行，人审可见（2026-09-23） |
